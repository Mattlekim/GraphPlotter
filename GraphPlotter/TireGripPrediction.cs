...
namespace GraphPlotter
{
    ...
    public struct TireGripAngleData
    {
        ...
        public float PredictAndCheck(float speed, out bool valid)
        {
            valid = true;
            if (speed > 20)
            {

                float prediction = Predict(speed);
                float pre_prediction = Predict(speed - 1);
                if (prediction == pre_prediction)
                    valid = false;
                return prediction;
            }
    ...
    public class TireGripPrediction
    {
        ...
        public float Predict(float speed, float steeringAngle)
        {
            ...
            float min = TireData[index - 1].SeeringAngle;
            float differnce = MathHelper.Distance(TireData[index].SeeringAngle, TireData[index - 1].SeeringAngle);
            differnce = 1 - (MathHelper.Distance(steeringAngle, min) / differnce);
